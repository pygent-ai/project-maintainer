...
description: Use when initializing or updating structured in-repository project-maintenance docs; exploring existing code; analyzing a full project or repository; documenting complete source symbol coverage, coverage maps, or health for every top-level function and class method; summarizing git history; or syncing docs after a feature, refactor, or bug fix.
---
...
3. If the artifact exists, read `.doc_project_maintainer/INDEX.md`, `.doc_project_maintainer/manifest.yaml`, `.doc_project_maintainer/project/build-plan.md`, and `.doc_project_maintainer/project/coverage-map.json` when present.
...
1. Create `project/build-plan.md` and refresh `project/coverage-map.json` with phases, scope, completed slices, pending slices, stale files, changed hashes, blocked questions, and integration notes.
...
5. Integrate slice outputs centrally by updating `manifest.yaml`, `INDEX.md`, cross-links, `project/coverage-map.json`, and `project/build-plan.md`.
...
2. Start with Explore Existing Project, then Build Large Artifacts In Slices, then continue selecting the next actionable pending slice from `project/coverage-map.json` and `project/build-plan.md` until the coverage closure audit has no actionable pending module, directory, flow, or code symbol slices.
3. An actionable pending slice is any stable tracked path, candidate project file, required flow, stable source file, required top-level function, or required class method that is not documented and not explicitly out of scope with a reason.
...
6. Do not mark code symbol coverage, project-wide coverage, sync status, or a full-repository goal `current` while any required source file lacks an inventory, any required top-level function or class method lacks an entry doc, or any entry doc lacks `Actual Role` and health fields.
...
6. When `--verify-docs` reports missing entry docs, missing `Actual Role`, or missing health fields, record those items as actionable pending code symbol slices.
...
9. Run `scripts/inventory_symbols.py` with `--verify-docs` and `--coverage-map-output`. Check whether every stable source file has a file-level code doc and symbol inventory, every top-level function has an entry doc with `Actual Role` and health fields, every method on every top-level class has an entry doc with `Actual Role` and health fields, and `project/coverage-map.json` has no actionable pending, stale, pending_review, not_checked, removed, or candidate project file work.
...
11. Record the audit source, unmapped paths, actionable pending slices, out-of-scope paths, untracked disposition, flow trace disposition, code symbol disposition, undocumented source files, undocumented top-level functions, undocumented class methods, and criteria to mark `current` in `project/build-plan.md`.
...
For complete or full-repository deliverables, also run `scripts/inventory_symbols.py` with `--verify-docs`. Treat any missing docs, missing health, missing `Actual Role`, or `requires_review` files as pending work, not success.
